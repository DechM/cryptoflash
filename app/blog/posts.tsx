...
export const blogPosts: BlogPost[] = [
  {
    ...
    content: (
      <>
        ...
        <ul>
          <li>Refresh Discord tiers: exploration alerts @ 60–65% progress; accumulation alerts only when inflows exceed $25K/30 min.</li>
          <li>Log alert-driven trades and set reminders to reassess if inflows stall. Use the takeaways below as your Monday stand-up agenda.</li>
          <li>
  ...
  },
  {
    ...
    content: (
      <>
        ...
        <p>
          When an alert fires ask three questions: is liquidity thick enough; does price action support the move; has this wallet delivered alpha before? Two “yes” answers mean the alert deserves a closer look.
        </p>
        ...
        <p>
          Build a checklist before acting. Confirm liquidity, review price action, pre-define position size. FOMO entries into shallow pools burn bankroll faster than any losing trade.
        </p>
        <p>
          Document how many alerts you need before acting. Some desks enter on the first buy from a pre-qualified wallet; others wait for confirmation. Consistency beats adrenaline.
        </p>
  ...
  },
  {
    ...
    content: (
      <>
        ...
        <p>
          Ultimate users should maintain dedicated profiles: an aggressive template (80% progress + 30% score jump in 30 minutes) and a conservative one (90% progress + stable inflows + Mayhem overlap).
        </p>
        ...
        <p>
          Pin playbooks like “If whale inflow &gt; $50K and progress &lt; 88%, open a 0.5 SOL scout position.” Clarity eliminates hesitation.
        </p>
        <h2>3. Execute & Review</h2>
        <p>
          Execution discipline means logging every trade—entry, exit, reason, outcome. CryptoFlash alerts become the audit trail for what triggered each move.
        </p>
        <p>
          Run weekly reviews: which alerts worked, which you ignored, and why. Adjust thresholds and SOPs so the signal stays sharp.
        </p>
        <p>
  ...
  },
  {
    ...
    content: (
      <>
        <p className="callout">
          Bonding curves price supply dynamically. Learn the math once and every KOTH unlock suddenly looks predictable.
        </p>
        ...
        <p>
          CryptoFlash tracks liquidity in SOL and USD so you see the implied market cap at every progress point. Benchmark against peers to decide if a curve is overheating.
        </p>
        <h2>2. Why KOTH Exists</h2>
        <p>
          King of the Hill (KOTH) is the battleground where curves near completion and teams fight for open liquidity. Sustained accumulation forces listings; weak conviction turns into exit liquidity.
        </p>
        <p>
          Monitor curve velocity (progress per minute) to spot organic demand versus orchestrated spikes. Sudden acceleration without matching inflows often signals a rug attempt.
        </p>
        <h2>3. Modelling Scenarios</h2>
        <p>
          Use CryptoFlash to simulate fill scenarios—adjust volume assumptions, whale inflows or team unlock events. The tool recalculates time-to-completion so you can plan entries or exits ahead of the crowd.
        </p>
        <p>
